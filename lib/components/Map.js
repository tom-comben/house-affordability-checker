...
const Map = (props) => {
  ...
  const [regions, setRegions] = useState(currentMap.paths);
  ...
  const pan = async (event) => {
    if (panParams.isPanning && readyToListen) {
      readyToListen = false;
      const offsetX = event.clientX - panParams.startMouseX;
      const offsetY = event.clientY - panParams.startMouseY;
      const newMinX =
        panParams.startMinX -
        offsetX * panParams.panRatio +
        panParams.zoomOffsetMinX;
      const newMinY =
        panParams.startMinY -
        offsetY * panParams.panRatio +
        panParams.zoomOffsetMinY;
      setViewbox({
        ...viewbox,
        minX: Math.max(
          panParams.offsetLimits.minX,
          Math.min(panParams.offsetLimits.maxX, newMinX)
        ),
        minY: Math.max(
          panParams.offsetLimits.minY,
          Math.min(panParams.offsetLimits.maxY, newMinY)
        ),
        latestUpdate: "pan",
      });
    }
